...
test_one_due_no_prior_state() {
    ...
    prev=$(read_fixture state_empty.json)
    got=$(diff_state "$current" "$prev")
    want='{"pings":[{"id":"lactic_acid","display":"Lactic acid training"}],"new_state":{"effective_date":"2026-05-13","seen_due":{"lactic_acid":true}}}'
...
test_flipped_false_after_log_same_day() {
    ...
    local current prev got want
    current='{"effective_date":"2026-05-13","reminders":[{"id":"lactic_acid","display":"Lactic acid training","interval_days":2,"last_done":"2026-05-13","days_since":0,"due":false,"not_before":null,"not_after":null}]}'
    prev=$(read_fixture state_one_seen_due.json)
    got=$(diff_state "$current" "$prev")
...
test_daily_reset_repings_overdue() {
    ...
    got=$(diff_state "$current" "$prev")
    want='{"pings":[{"id":"lactic_acid","display":"Lactic acid training"}],"new_state":{"effective_date":"2026-05-14","seen_due":{"lactic_acid":true}}}'
...
test_mixed_new_edge_seen_and_just_logged() {
    ...
    got=$(diff_state "$current" "$prev")
    want='{"pings":[{"id":"lactic_acid","display":"Lactic acid training"}],"new_state":{"effective_date":"2026-05-13","seen_due":{"lactic_acid":true,"brush_evening":false,"weigh_in":true}}}'
